...
import com.waterphage.mixin.ExampleMixin;
...
public final class ModGenerator extends ChunkGenerator {


	public static final Codec<ModGenerator> CODEC = RecordCodecBuilder.create(
		instance -> instance.group(
					BiomeSource.CODEC.fieldOf("biome_source").forGetter(generator -> generator.biomeSource),
					ChunkGeneratorSettings.REGISTRY_CODEC.fieldOf("settings").forGetter(generator -> generator.settings)
				)
				.apply(instance, instance.stable(ModGenerator::new))
	);
	private static final BlockState AIR = Blocks.AIR.getDefaultState();
	private final RegistryEntry<ChunkGeneratorSettings> settings;
	private final Supplier<AquiferSampler.FluidLevelSampler> fluidLevelSampler;

	public ModGenerator(BiomeSource biomeSource, RegistryEntry<ChunkGeneratorSettings> settings) {
		super(biomeSource);
	...
	private void populateBiomes(Blender blender, NoiseConfig noiseConfig, StructureAccessor structureAccessor, Chunk chunk) {
		...
		chunk.populateBiomes(
			biomeSupplier, ((ExampleMixin.createMultiNoiseSamplerInvoker)chunkNoiseSampler).createMultiNoiseSampler(noiseConfig.getNoiseRouter(), this.settings.value().spawnTarget())
		);
	}
		...
		if (l <= 0) {
			...
			for(int t = l - 1; t >= 0; --t) {
				...
				for(int u = i - 1; u >= 0; --u) {
					...
					//((ExampleMixin.sampleBlockStateInvoker)chunkNoiseSampler).sampleBlockState
					BlockState blockState = ((ExampleMixin.sampleBlockStateInvoker)chunkNoiseSampler).sampleBlockState();
	...
	public void carve(
		...
		CarverContext carverContext = new CarverContext(
			...
			chunkNoiseSampler,
			noiseConfig,
			((ChunkGeneratorSettings)this.settings.value()).surfaceRule()
		);
		...
		if (k <= 0) {
			...
			return CompletableFuture.supplyAsync(
				...
				.whenCompleteAsync((chunkx, throwable) -> {
					for(ChunkSection chunkSectionxx : set) {
						chunkSectionxx.unlock();
					}
				}, executor);
		}
	}
	...
	private Chunk populateNoise(Blender blender, StructureAccessor structureAccessor, NoiseConfig noiseConfig, Chunk chunk, int minimumCellY, int cellHeight) {
		...
		int k = ((ExampleMixin.getHorizontalCellBlockCountInvoker)chunkNoiseSampler).getHorizontalCellBlockCount();
		int l = ((ExampleMixin.getVerticalCellBlockCountInvoker)chunkNoiseSampler).getVerticalCellBlockCount();
		int m = 16 / k;
		int n = 16 / k;
